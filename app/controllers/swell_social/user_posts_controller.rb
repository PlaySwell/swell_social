module SwellSocial
	class UserPostsController < ApplicationController
		...
		def create
			@post = UserPost.new( parent_obj_id: @parent_obj.id, parent_obj_type: @parent_obj.class.name, user: current_user, subject: params[:subject], content: params[:content], status: ( params[:draft] ? 'draft' : 'active' ) )

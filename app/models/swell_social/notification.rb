module SwellSocial
	...
	class Notification < ActiveRecord::Base
		...
		def actor_list
			if children_count > 0
				User.joins("INNER JOIN (#{self.children.group(:actor_id).select(:actor_id, 'MIN(created_at) created_at').reorder('').to_sql}) user_post_children ON user_post_children.user_id = users.id").order('user_post_children.created_at ASC')
			else
